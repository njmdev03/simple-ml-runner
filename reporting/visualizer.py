...
class Visualizer:
            ...
            if 'epoch' in metric.lower():
                ...
                for i, part in enumerate(parts):
                    if part == 'epoch' and i + 1 < len(parts):
                        try:
                            epoch = int(parts[i + 1])
                            if 'training' in metric.lower():
                                training_times[epoch] = value
                            elif 'testing' in metric.lower():
    ...
    def plot_sample_predictions(self, model, dataset, device, 
        ...
        # If external ax is provided, we can only plot one sample or a pre-made grid
        # For simplicity, if ax is provided, we just pick ONE sample to show on that ax
        if ax is not None:
