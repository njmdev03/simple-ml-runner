...
def _build_dataclass(schema, data: dict):
    ...
    for f in fields(schema):
        ...
        if key in data:
            value = data[key]
            kwargs[key] = _convert_value(f, value)
        else:
...
def _convert_value(field, value):
    ...
    if type_hint is Variant:
        ...
        if name not in field.default.mapping:
            raise ValueError(f"Unknown variant: {name}")

        cls = field.default.mapping[name]
        obj = _build_dataclass(cls, payload)
    ...
    if origin is list:
        ...
        return [_convert_value(inner_type, v) for v in value]

    if origin is dict:
        k_type, v_type = get_args(type_hint)
        return {
            _convert_value(k_type, k): _convert_value(v_type, v)
